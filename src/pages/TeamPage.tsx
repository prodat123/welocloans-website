...
const team = [
  {
    ...
    phone: '(626) 374-8775',
    email: 'warrenc.welocloans@gmail.com',
  ...
  },
  {
    ...
    phone: '—',
    email: '—',
    bio: 'We\'re growing our team. Reach out if you\'re interested in joining WELOC Loans.',
    langs: [],
    initials: '?',
    color: 'bg-slate-300',
    placeholder: true,
    ...
    <div className="bg-transparent min-h-screen pt-28 pb-24">
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="mb-16 max-w-2xl">
          <p className="text-xs font-bold tracking-[0.12em] uppercase text-blue-600 mb-3">Our Team</p>
          <h1 className="text-5xl font-extrabold text-slate-900 tracking-tight mb-4">
            People you can actually talk to.
          </h1>
          <p className="text-lg text-slate-500 leading-relaxed">
            Our loan originators are real people who will walk you through every step —
            in your language, at your pace, with zero pressure.
          </p>
        </div>
          ...
          {team.map((member) => (
            ...
            >
              ...
              <div className="bg-slate-50 px-6 pt-8 pb-6 flex items-center gap-5">
                <div className={`w-20 h-20 rounded-2xl ${member.color} flex items-center justify-center shrink-0`}>
                  <span className={`text-2xl font-extrabold ${member.placeholder ? 'text-slate-400' : 'text-white'}`}>
                    {member.initials}
                  </span>
                </div>
                <div>
                  <h2 className={`text-lg font-extrabold ${member.placeholder ? 'text-slate-400' : 'text-slate-900'}`}>
                    {member.name}
                  </h2>
                  ...
                  {member.langs.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-2">
                      {member.langs.map(l => (
                        <span key={l} className="text-[10px] font-semibold bg-blue-50 text-blue-700 px-2 py-0.5 rounded-full">
                          {l}
                        </span>
                      ))}
                    </div>
              ...
              <div className="px-6 pb-6">
                <p className={`text-sm leading-relaxed mb-5 ${member.placeholder ? 'text-slate-400 italic' : 'text-slate-500'}`}>
                  {member.bio}
                </p>
        ...
        <div className="mt-16 bg-blue-700 rounded-2xl p-8 flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <h3 className="text-xl font-extrabold text-white mb-1">Ready to get started?</h3>
            <p className="text-blue-200 text-sm">Pick up the phone or send us a message — we'll respond within one business day.</p>
          </div>
          <a
            href="/#contact"
