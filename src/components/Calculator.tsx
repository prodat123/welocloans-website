...
const PROGRAMS = [
  {
    ...
    shortName: "LIPA",
    name: "LA City LIPA",
    tagline: "Up to $140,000 silent second — $0/month on assistance",
    firstLoan: (price: number) => Math.max(price * 0.14, price * 0.99 - 140000),
    buyerCash: (price: number) => price * 0.01,
    aid: (price: number) => Math.min(140000, price * 0.85),
  ...
  },
  {
    ...
    tagline: "State covers 20% of purchase price — no interest or monthly payment",
  ...
  return (
    <div className="mb-5">
      <div className="flex justify-between items-center mb-2">
        ...
        >
          {display ?? (unit === "$" ? formatCurrency(value) : `${value}${unit}`)}
        </span>
      </div>
      <input
        type="range"
...
export default function Calculator() {
  ...
  const pi = useMemo(() => calcMonthly(loanAmount, rate, term), [loanAmount, rate, term]);
  ...
  const afterPi = useMemo(() => calcMonthly(afterLoan, rate, term), [afterLoan, rate, term]);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start mb-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <div className="flex justify-between items-center mb-1">
                  <span className="text-sm font-medium text-slate-600">Tax / mo</span>
                  <span className="text-sm font-bold text-blue-700 bg-blue-50 px-2 py-0.5 rounded-lg">{formatCurrency(tax)}</span>
                </div>
                <p className="text-xs text-slate-400">1.25% annually</p>
              </div>
              <div>
                <div className="flex justify-between items-center mb-1">
                  <span className="text-sm font-medium text-slate-600">Insurance / mo</span>
                  <span className="text-sm font-bold text-blue-700 bg-blue-50 px-2 py-0.5 rounded-lg">{formatCurrency(insurance)}</span>
                </div>
          ...
          <div className="flex flex-col gap-4">
            <div className="bg-white border border-slate-200 rounded-2xl p-6">
              ...
              <p className="text-xs text-slate-500 text-center">{prog.tagline}</p>
            </div>

            {/* Quick stats */}
            <div className="bg-white border border-slate-200 rounded-2xl p-6 grid grid-cols-2 gap-4">
              {[
                { label: "Standard loan amount", value: formatCurrency(loanAmount) },
                { label: `${prog.shortName} first loan`, value: formatCurrency(afterLoan) },
                { label: "Your cash at closing", value: formatCurrency(beforeCash) },
                { label: `Cash with ${prog.shortName}`, value: formatCurrency(afterCash) },
              ].map((s) => (
                <div key={s.label}>
              ...
              <div className="col-span-2 border-t border-slate-100 pt-3 mt-1">
                <p className="text-[11px] text-slate-400 mb-0.5">{prog.aidLabel}</p>
                <div className="flex items-center justify-between">
                  <p className="text-base font-extrabold text-slate-800">{formatCurrency(afterAid)}</p>
                  <span className="text-[10px] font-semibold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full">{prog.aidNote}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="rounded-2xl overflow-hidden border border-slate-200 shadow-sm">
          <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_1fr]">
            ...
            <div className="bg-slate-100 p-8 flex flex-col items-center justify-center text-center">
              ...
              <div className="w-full max-w-[200px] space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold text-slate-600">{formatCurrency(pi)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-400">Tax + Insurance</span>
                  <span className="font-semibold text-slate-600">{formatCurrency(tax + insurance)}</span>
                </div>
                <div className="flex justify-between border-t border-slate-200 pt-2">
                  <span className="text-slate-400">Down payment</span>
                  <span className="font-semibold text-slate-600">{formatCurrency(beforeCash)}</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-blue-700 flex flex-col items-center justify-center px-6 py-8 gap-3">
              <svg viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="w-6 h-6 opacity-60 hidden md:block">
                <line x1="5" y1="12" x2="19" y2="12" /><polyline points="12 5 19 12 12 19" />
              </svg>
              <div className="text-center">
                {monthlySavings > 5 ? (
                  <>
                    <p className="text-blue-200 text-[10px] font-bold uppercase tracking-widest mb-1">You save</p>
                    <p className="text-3xl font-extrabold text-white leading-none">{formatCurrency(monthlySavings)}</p>
                    <p className="text-blue-200 text-xs mt-1">per month</p>
                    <p className="text-blue-300 text-xs mt-0.5">{formatCurrency(monthlySavings * 12)}/yr</p>
                  </>
                ) : (
                  <>
                    <p className="text-blue-200 text-[10px] font-bold uppercase tracking-widest mb-1">You keep</p>
                    <p className="text-3xl font-extrabold text-white leading-none">{formatCurrency(Math.max(0, cashSavings))}</p>
            ...
            <div className="bg-blue-700 p-8 flex flex-col items-center justify-center text-center">
              ...
              <div className="w-full max-w-[200px] space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold text-white">{formatCurrency(afterPi)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-blue-300">Tax + Insurance</span>
                  <span className="font-semibold text-white">{formatCurrency(tax + insurance)}</span>
                </div>
                <div className="flex justify-between border-t border-blue-600 pt-2">
                  <span className="text-blue-300">Cash at closing</span>
                  <span className="font-semibold text-white">{formatCurrency(afterCash)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-blue-300">{prog.aidLabel}</span>
                  <span className="font-semibold text-white">{formatCurrency(afterAid)}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-800 px-8 py-4 flex flex-col sm:flex-row items-center justify-between gap-3">
            <p className="text-xs text-slate-400">
              * Illustrative estimates only. Tax, insurance, and PMI may vary. Contact us for an official quote.
            </p>
            <a
              href="#contact"
