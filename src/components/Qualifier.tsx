...
const programInfo: Record<string, { name: string; tagline: string; color: string }> = {
  hop: { name: 'HOP Program', tagline: 'Down payment assistance — deferred loan', color: 'border-blue-400 bg-blue-50' },
  lipa: { name: 'LIPA Program', tagline: 'Up to $140,000 silent second mortgage', color: 'border-green-400 bg-green-50' },
  calhfa: { name: 'CalHFA Dream for All', tagline: 'Up to 20% covered — statewide', color: 'border-purple-400 bg-purple-50' },
  santaAna: { name: 'Santa Ana Program', tagline: 'City of Santa Ana assistance (verify funds)', color: 'border-orange-400 bg-orange-50' },
  gardenGrove: { name: 'Garden Grove Program', tagline: 'City of Garden Grove assistance (verify funds)', color: 'border-orange-400 bg-orange-50' },
}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
          ...
          <div className="space-y-3">
            {questions.map(q => (
              ...
              >
                <span className={`mt-0.5 w-5 h-5 shrink-0 rounded-md border-2 flex items-center justify-center transition-colors ${
                  checked[q.id] ? 'bg-blue-700 border-blue-700' : 'border-slate-300 bg-white'
                }`}>
                  {checked[q.id] && (
                ...
                <div>
                  <p className={`text-sm font-semibold leading-snug ${checked[q.id] ? 'text-blue-900' : 'text-slate-800'}`}>
                    {q.label}
                  </p>
                  <p className="text-xs text-slate-400 mt-0.5 leading-relaxed">{q.tip}</p>
                </div>
              </button>
            ))}
          </div>
          ...
          <div className="lg:sticky lg:top-28">
            <div className="bg-slate-50 border border-slate-200 rounded-2xl p-7">
              <h3 className="text-base font-extrabold text-slate-900 mb-1">Programs you may qualify for</h3>
              <p className="text-xs text-slate-400 mb-6">
                {anyChecked ? 'Based on your selections:' : 'Check the boxes on the left to see your results.'}
              </p>

              {!anyChecked && (
                <div className="flex flex-col items-center py-10 text-center">
                  <span className="text-4xl mb-3">☑️</span>
                  <p className="text-sm text-slate-400">Your matching programs will appear here as you check items on the left.</p>
                </div>
              )}
              ...
              {anyChecked && qualified.length === 0 && (
                <div className="flex flex-col items-center py-8 text-center">
                  ...
                  <p className="text-sm font-semibold text-slate-700 mb-1">No programs matched yet</p>
                  <p className="text-xs text-slate-400 max-w-xs">Keep checking boxes — you may still qualify once more criteria are met. Contact us and we'll look at every option for you.</p>
                </div>
              )}
              ...
              <div className="space-y-3">
                {qualified.map(key => {
                  ...
                  return (
                    <div key={key} className={`border-2 rounded-xl px-4 py-3.5 ${p.color}`}>
                      <div className="flex items-center gap-2 mb-0.5">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#2a7c8a" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                          <polyline points="20 6 9 17 4 12"/>
                        </svg>
              ...
              <p className="text-[10px] text-slate-400 text-center mt-4">
                This is a general eligibility indicator, not a guarantee. A loan officer will confirm your exact eligibility.
              </p>
            </div>
          </div>
        </div>
